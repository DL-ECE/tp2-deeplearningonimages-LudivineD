...
Original file is located at
    https://colab.research.google.com/drive/1cNPF3OxlTuWs9ePb1Y3eXUCrocLPeGQW
...
#test = build_image_like_tensor(2,3,4,5)
#print(test)

# Create 3 different tensors with the above function containing different value between [0,255]
# Uncomment the 3 line below and complete with your answer 

white_like = build_image_like_tensor(100,100,3,1)   #1 = 255/255
gray_like = build_image_like_tensor(100,100,3,127/255)
black_like = build_image_like_tensor(100,100,3,0)   #0 = 0/255
...
red_like = build_image_like_tensor(100,100,3,[1,0,0])     #1 = 1/255
green_like = build_image_like_tensor(100,100,3,[0,1,0])
blue_like = build_image_like_tensor(100,100,3,[0,0,1])
...
def convolution_forward_numpy(image, kernel):
    # YOUR CODE HERE 
    mat = np.zeros((image.shape[0]-kernel.shape[0]+1, image.shape[1]-kernel.shape[1]+1))
    for x in range(image.shape[1]-2):
        for y in range(image.shape[0]-2):
            mat[y, x] = (kernel * image[y:(y+3), x:(x+3)]).sum()
    return mat
...
def convolution_forward_torch(image, kernel):
    ...
    conv = torch.nn.Conv2d(1, 1, kernel.shape[0])
    output = conv(image)
    return output

    
output_image_bis = convolution_forward_torch(image, kernel)
display_image(output_image_bis)
...
print(fmnist_train.dataset.targets[0])
